...
SECTOR_REPRESENTATIVE_STOCKS = {
    ...
    '반도체소재': {
        'core_stocks': ['007660', '457190', '357780', '005290', '093370', '213420', '319660'],  # 457190 추가
        'names': ['이수페타시스', '이수스페셜티케미컬', '솔브레인', '동진세미켐', '후성', '덕산네오룩스', '피에스케이'],  # 추가
        'keywords': ['반도체', '소재', '특수가스', '세정액', 'OLED', '전자재료', '웨이퍼', '반도체장비', '스페셜티케미컬', '반도체화학']  # 키워드 확장
    },
...
def calculate_market_adaptive_partial_sell(stock_data, position, target_config):
    ...
    try:
        ...
        if market_condition == 'bearish_volatile':
            ...
            # 🌞 강세장/보통: 개선된 분할매도 ⭐ 여기를 교체!
            partial_strategies = calculate_enhanced_partial_strategies(  # 👈 기존 calculate_normal_partial_strategies에서 교체
                profit_rate, total_amount, is_overheated, drawdown_from_high, min_profit_threshold
            )            
...
def calculate_surge_adaptive_partial_sell(stock_data, position, target_config):
    ...
    try:
        ...
        if surge_strength == 'EXPLOSIVE':
            ...
            # ⭐ 중간 상승에서 개선된 로직 적용
            partial_strategies = calculate_enhanced_partial_strategies(  # 👈 기존 함수 대신 교체
                profit_rate, total_amount, is_overheated, drawdown_from_high, min_profit_threshold
            )
            ...
            # ⭐ 약한 상승에서도 개선된 로직 적용 (한화시스템 케이스 대응)
            partial_strategies = calculate_enhanced_partial_strategies(  # 👈 기존 함수 대신 교체
                profit_rate, total_amount, is_overheated, drawdown_from_high, min_profit_threshold
            )            
