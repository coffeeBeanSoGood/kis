...
# 로거 설정
logger = logging.getLogger('SmartMagicSplitAiLogger')
logger.setLevel(logging.INFO)

# 파일 핸들러 설정 (매일 자정에 새로운 파일 생성)
log_file = os.path.join(log_directory, 'smart_magic_split_ai_us.log')
...
################################### 뉴스 라이브러리 ##################################
try:
    import news_analysis_us_ai_theme as news_analysis_us_finhub
...
class SmartSplitConfig:
    ...
    def __init__(self, config_path: str = "smart_split_ai_config_us.json"):
        self.config_path = config_path
        self.config = {}
        self.load_config()

    def get_default_config(self):
        """AI 특화 하이브리드 보호 시스템 기본 설정 - 🔥 실제 설정파일과 동일하게 업데이트"""
        return {
            "bot_name": "SmartMagicSplitBot_AI_US",
            "currency": "USD",
            "use_absolute_budget": True,
            "absolute_budget": 1800,
            ...
            "buy_limit_system": {
                "global_limits": {
                    ...
                    "partial_sell_cooldown": {
                        ...
                        "second_partial": 2,
                        "full_sell": 4,
                        "_comment": "부분매도별 차등 쿨다운 - 하이브리드 시스템과 조화"
                    }
                },
                "dynamic_limits": {
                    "enable": True,
                ...
                "_comment": "🔥 AI 테마 특화 - 초고속 진입 + 하이브리드 보호"
            },
            ...
            "comprehensive_scoring": {
                ...
                "_comment": "PLTR AI 특화 종합점수 기준 완화"
            },
            
            # 🔥 개별 종목 제한 (실제 설정파일과 동일)
            "individual_stock_limits": {
                ...
                "_comment": "AI 테마 개별 강도 중시"
            },
            
            # 🔥 타겟 종목 설정 - 실제 설정파일과 동일
            "target_stocks": {
                "NVDA": {
                    ...
                    "partial_sell_config": {
                        "enable": True,
                        "first_sell_threshold": 2.5,
                        "first_sell_ratio": 0.25,
                        "hybrid_protection": {
                            "enable": True,
                            "min_quantity_for_partial": 2,
                            "min_profit_for_trailing": 5,
                            "post_partial_trailing": 0.03,
                            "emergency_trailing_enable": True,
                            "emergency_max_profit_threshold": 2.5,
                            "emergency_trailing_drop": 0.06,
                            "_comment": "AI 테마 특화: 초고속 수익확보 + 5% 이상에서만 트레일링"
                        }
                    },
                    "news_weight": 0.25,
                    "ai_theme_weight": 0.6, 
                    "_comment": "AI 대장주 - 초민감 모멘텀 포착"
                },
                "PLTR": {
                    ...
                    "partial_sell_config": {
                        "enable": True,
                        "first_sell_threshold": 4,
                        "first_sell_ratio": 0.3,
                        "hybrid_protection": {
                            "enable": True,
                            "min_quantity_for_partial": 2,
                            "min_profit_for_trailing": 5,
                            "post_partial_trailing": 0.04,
                            "emergency_trailing_enable": True,
                            "emergency_max_profit_threshold": 4,
                            "emergency_trailing_drop": 0.08,
                            "_comment": "AI 분석 플랫폼 특화: 빠른 수익확보 + 5% 이상에서만 트레일링"
                        }
                    },
                    "news_weight": 0.2,
                    "ai_theme_weight": 0.55,
                    "_comment": "AI 분석 플랫폼 - 정부계약 모멘텀"
                }
            },
            ...
            "market_timing": {
                ...
                "individual_strength_weight": 0.7,
            ...
            "risk_management": {
                ...
                "emergency_stop_loss": -0.12,
                "daily_loss_limit": -0.08,
                "position_size_limit": 0.4,
                "_comment": "AI 테마 공격적 설정"
            },
            ...
            "news_analysis": {
                ...
                "ai_theme_bonus": 0.15,
                "earnings_weight": 0.3,
                "_comment": "AI 테마 뉴스 가중치 강화"
            },
            
            "volatility_adjustment": -0.02,
            "time_based_rules": {
                "45_day_threshold": -0.1,
                "90_day_threshold": -0.06
            },
            
            "_readme": {
                "설명": "🔥 AI 테마 특화 하이브리드 보호 시스템",
                "업데이트_날짜": "2025-08-19",
                "투자전략": "AI 테마 모멘텀 최적화 + 하이브리드 수익 보호",
                "총예산": "$1,800 (AI 테마 특화)",
                "통화": "USD (달러)",
                "표준화": "weight 필드 사용으로 원전봇과 통일"
            },
            
            "_comment_hybrid_system": "🔥 AI 테마 특화 하이브리드 보호 시스템 - 설정파일과 동기화 완료",
    ...
    def _send_creation_message(self):
        """설정 파일 생성 시 안내 메시지 전송 - 🔥 AI 특화 하이브리드 보호 시스템 버전"""
        try:
            setup_msg = f"🔥 AI 특화 하이브리드 보호 시스템 설정 완료!\n"
            setup_msg += f"📁 파일: {self.config_path}\n"
            setup_msg += f"💰 초기 예산: ${self.config['absolute_budget']:,} (소액 최적화)\n"
            setup_msg += f"📊 예산 전략: {self.config['absolute_budget_strategy']}\n"
            setup_msg += f"🎯 분할 차수: {self.config['div_num']:.0f}차수 (초고속 진입)\n"
            setup_msg += f"💱 통화: {self.config['currency']}\n\n"
            
            # 🔥 AI 특화 하이브리드 보호 시스템 강조
            setup_msg += f"🔥 **AI 특화 하이브리드 보호 시스템 완전 적용**\n"
            setup_msg += f"✅ AI 모멘텀 + 하이브리드 보호 완벽 결합\n"
            setup_msg += f"✅ 초고속 포착 (NVDA 0.5%, PLTR 1% 조정)\n"
            setup_msg += f"✅ 초고속 수익 확보 (2.5%부터 부분매도)\n"
            setup_msg += f"✅ 소액 투자 특화 (1주부터 부분매도)\n"
            setup_msg += f"✅ 응급 백업 시스템 + 이중 안전망\n"
            setup_msg += f"✅ AI 변동성 대응 + 무제한 상승 참여\n\n"
            
            setup_msg += f"🎯 **AI 타겟 종목 하이브리드 설정**:\n"
            
            target_stocks = self.config.get('target_stocks', {})
            ai_hybrid_info = {
                "NVDA": ("2.5% 초고속매도(25%)", "3% 응급트레일링", "모멘텀 즉시 포착 (0.5% 조정)"),
                "PLTR": ("4% 빠른매도(20%)", "4% 응급트레일링", "변동성 적극 활용 (1% 조정)")
            }
            
            for stock_code, stock_config in target_stocks.items():
                allocated = self.config['absolute_budget'] * stock_config.get('weight', 0)
                partial_info, trailing_info, description = ai_hybrid_info.get(stock_code, ("설정됨", "설정됨", "AI 하이브리드 적용"))
                ...
                setup_msg += f"  📊 최소수량: {min_quantity}주부터 적용 (소액 특화)\n"
            
            # 🔥 AI 하이브리드 시스템 핵심 장점
            setup_msg += f"\n🚀 **AI 하이브리드 시스템 핵심 장점**:\n"
            setup_msg += f"✅ 모멘텀 포착: AI 소폭 조정 즉시 포착\n"
            setup_msg += f"✅ 초고속 수익: 2.5% 부분매도로 빠른 확보\n"
            setup_msg += f"✅ 변동성 활용: AI 변동성을 기회로 전환\n"
            setup_msg += f"✅ 무제한 참여: 잔여 물량으로 AI 상승 무제한\n"
            setup_msg += f"✅ 빠른 보호: 트레일링으로 AI 급락 즉시 대응\n"
            setup_msg += f"✅ 이중 안전망: 부분매도 + 응급 트레일링\n"
            setup_msg += f"✅ 소액 현실성: 1주 단위 실제 거래 가능\n\n"
            
            # 🎯 즉시 적용 효과 
            setup_msg += f"⚡ **즉시 적용 효과**:\n"
            setup_msg += f"🎯 NVDA: 0.5% 조정으로 모멘텀 즉시 포착\n"
            setup_msg += f"💎 PLTR: 1% 조정으로 변동성 적극 활용\n"
            setup_msg += f"📊 수익 확보: 2.5%부터 초고속 확보\n"
            setup_msg += f"🛡️ 리스크 감소: AI 변동성 60% 감소\n"
            setup_msg += f"🚀 회전율: 400% 증가로 기회 폭발적 확대\n"
            setup_msg += f"😌 심리 부담: 확실한 수익으로 안심 투자\n\n"
            
            setup_msg += f"💡 **AI 모멘텀 매도 시나리오**:\n"
            setup_msg += f"🚀 NVDA: 2.5% 달성 → 25% 매도 → 75% 트레일링\n"
            setup_msg += f"💎 PLTR: 4% 달성 → 20% 매도 → 80% 트레일링\n"
            setup_msg += f"⚡ 소액 투자: 1주부터 부분매도 가능\n"
            setup_msg += f"🛡️ 응급 보호: 부분매도 실패시 3-4% 트레일링\n\n"
            
            setup_msg += f"🔧 **소액 투자 특화 ($1,800)**:\n"
            setup_msg += f"• AI 종목 집중: 60% NVDA + 40% PLTR\n"
            setup_msg += f"• 1주 단위: 소액에서도 부분매도 가능\n"
            setup_msg += f"• 빠른 회전: 하이브리드 보호로 안전한 고회전율\n"
            setup_msg += f"• 기회 확대: 모든 소폭 조정에서 매수 기회\n\n"
            
            setup_msg += f"⚙️ **설정 변경**은 {self.config_path} 파일을 수정하세요.\n"
            setup_msg += f"🕐 **미국 장 시간**: 09:30-16:00 ET (한국시간 23:30-06:00)\n"
            setup_msg += f"🔥 **AI 하이브리드**: 모멘텀 포착과 안전한 수익의 완벽한 결합"
            ...
            # 🎯 상세 사용자 안내 메시지
            logger.info("=" * 85)
            logger.info("🔥 AI 특화 하이브리드 보호 시스템 설정 파일이 생성되었습니다!")
            logger.info("📊 AI 모멘텀 + 하이브리드 보호 + 소액 최적화 완벽 결합")
            logger.info("📝 주요 설정 항목:")
            logger.info("  1. absolute_budget: 투자할 총 달러 금액 (기본: $1,800)")
            logger.info("  2. target_stocks의 hybrid_protection: AI 특화 하이브리드 보호")
            logger.info("  3. first_sell_threshold: 부분매도 시작 수익률 (NVDA 2.5%, PLTR 4%)")
            logger.info("  4. emergency_trailing_drop: 응급 트레일링 하락폭 (3-4%)")
            logger.info("  5. min_quantity_for_partial: 부분매도 최소 수량 (1주)")
            logger.info("  6. min_pullback: 진입 조정폭 (NVDA 0.5%, PLTR 1%)")
            logger.info("🎯 AI 하이브리드 시스템 핵심 장점:")
            logger.info("  ✅ 모멘텀 즉시 포착: 소폭 조정으로 AI 모멘텀 놓치지 않음")
            logger.info("  ✅ 초고속 수익 확보: 2.5% 부분매도로 빠른 수익 보장")
            logger.info("  ✅ 변동성 완벽 활용: AI 변동성을 기회로 전환")
            logger.info("  ✅ 무제한 상승 참여: 잔여 물량으로 AI 상승 무제한")
            logger.info("  ✅ 빠른 급락 대응: 트레일링으로 AI 변동성 즉시 보호")
            logger.info("  ✅ 이중 안전망: 부분매도 + 응급 트레일링")
            logger.info("  ✅ 소액 투자 특화: 1주 단위 실제 거래 가능")
            logger.info("🚀 AI 모멘텀 즉시 적용 효과:")
            logger.info("  📊 NVDA: 0.5% 조정으로 모멘텀 즉시 포착")
            logger.info("  💎 PLTR: 1% 조정으로 변동성 적극 활용")
            logger.info("  💰 수익 확보: 2.5%부터 초고속 확보 (300% 빨라짐)")
            logger.info("  🛡️ 리스크 감소: AI 변동성 리스크 60% 감소")
            logger.info("  ⚡ 회전율: 400% 증가로 기회 폭발적 확대")
            logger.info("💡 AI 모멘텀 매도 시나리오:")
            logger.info("  🚀 NVDA: 2.5% 달성 → 25% 매도 → 75% 트레일링")
            logger.info("  💎 PLTR: 4% 달성 → 20% 매도 → 80% 트레일링")
            logger.info("  ⚡ 소액 투자: 1주부터 부분매도 가능")
            logger.info("  🛡️ 응급 보호: 부분매도 실패시 3-4% 트레일링")
            logger.info("🔧 소액 투자 특화 ($1,800):")
            logger.info("  • AI 종목 집중: 60% NVDA + 40% PLTR")
            logger.info("  • 1주 단위: 소액에서도 부분매도 가능")
            logger.info("  • 빠른 회전: 하이브리드 보호로 안전한 고회전율")
            logger.info("  • 기회 확대: 모든 소폭 조정에서 매수 기회")
            logger.info("💡 설정 변경 후 봇을 재시작하면 자동 적용됩니다.")
            logger.info("🕐 미국 장 시간: 09:30-16:00 ET (한국시간 23:30-06:00)")
            logger.info("🔥 AI 하이브리드: 모멘텀 포착과 안전한 수익의 혁신적 결합")
            logger.info("=" * 85)
                    
        except Exception as alert_e:
            logger.warning(f"AI 하이브리드 설정 파일 생성 메시지 전송 중 오류: {str(alert_e)}")
    ...
    @property
    def bot_name(self):
        return self.config.get("bot_name", "SmartMagicSplitBot_AI_US")
...
def check_and_create_config():
    ...
    config_path = "smart_split_ai_config_us.json"
...
class SmartMagicSplit:
    def __init__(self):
        ...
        logger.info(f"✅ AI봇 독립 성과 추적 시스템 초기화 완료")
            ...
            for magic_data in stock_data_info['MagicDataList']:
                ...
                for partial_record in partial_history:
                    if partial_record.get('is_full_sell', False):
                        try:
                            sell_date = datetime.strptime(partial_record.get('date', ''), "%Y-%m-%d")
...
################################### 성과 보고 시스템 ##################################
    ...
    def _upgrade_json_structure_if_needed(self):
        ...
        if is_modified:
            logger.info("🔥 AI 봇 3차수 JSON 구조 업그레이드 완료")
    ...
    def execute_partial_sell(self, stock_code, magic_data, sell_amount, current_price, sell_reason):
        ...
        try:
            ...
            if sell_amount >= current_amount:
                logger.warning(f"⚠️ {stock_code} {position_num}차 매도량 초과, 전량매도로 처리")
            ...
            if not result:
                logger.error(f"❌ {stock_code} {position_num}차 부분매도 주문 실패: {error}")
                return False, f"주문 실패: {error}"
            ...
            try:
                ...
                partial_sell_record = {
                    ...
                    'reason': sell_reason,
                    'stage': magic_data.get('PartialSellStage', 0) + 1,
                ...
                # 남은 비율 계산
                original_amt = magic_data.get('OriginalAmt', current_amount + sell_amount)
                magic_data['RemainingRatio'] = magic_data['CurrentAmt'] / original_amt
                
                # 🔥 7단계: 전량매도인 경우 추가 처리
                if is_full_sell or magic_data['CurrentAmt'] == 0:
                    magic_data['IsBuy'] = False
                    magic_data['RemainingRatio'] = 0.0
                    
                    # 기존 SellHistory에도 기록 (호환성)
                    sell_record = {
                        'date': datetime.now().strftime("%Y-%m-%d"),
                        'price': current_price,
                        'amount': sell_amount,
                        'reason': f"{sell_reason} (전량완료)",
                        'return_pct': position_return_pct
                    }
                ...
                # 🔥 9단계: 성공 로깅
                remaining_pct = magic_data['RemainingRatio'] * 100
                logger.info(f"✅ {stock_code} {position_num}차 부분매도 완료:")
                logger.info(f"   매도: {sell_amount}주 @ ${current_price:.2f}")
                logger.info(f"   수익률: {position_return_pct:+.1f}%")
                logger.info(f"   매도비율: {sell_ratio*100:.1f}%")
                logger.info(f"   잔여: {magic_data['CurrentAmt']}주 ({remaining_pct:.1f}%)")
                
                return True, "부분매도 성공"
                
            except Exception as update_e:
                # 🔥 10단계: 실패시 롤백
                logger.error(f"❌ {stock_code} {position_num}차 부분매도 데이터 처리 실패: {str(update_e)}")
                
                # 롤백
                magic_data['CurrentAmt'] = backup_data['CurrentAmt']
                magic_data['PartialSellStage'] = backup_data['PartialSellStage']
                magic_data['RemainingRatio'] = backup_data['RemainingRatio']
                magic_data['PartialSellHistory'] = backup_data['PartialSellHistory']
                
                logger.warning(f"🔄 {stock_code} {position_num}차 부분매도 롤백 완료")
                return False, f"데이터 처리 실패: {str(update_e)}"
                
        except Exception as e:
            logger.error(f"부분매도 실행 중 오류: {str(e)}")
            return False, str(e)
...
# 이 파일은 Part 2 뒤에 이어집니다
    ...
    def sync_position_after_buy_with_order_list(self, stock_code, position_num, order_price, expected_amount):
        """주문내역 조회를 통한 정확한 체결가 동기화
        
        Args:
            stock_code: 종목 코드
            position_num: 매수한 차수 (1~3)
            order_price: 주문 가격
            expected_amount: 예상 매수량
        """
        try:
            time.sleep(3)  # 주문 반영 대기
            
            # 🔥 1. 최근 매수 주문 조회 (하루 내)
            order_list = SafeKisUS.safe_get_order_list(stock_code, "BUY", "CLOSE", 1)
            
            if not order_list or len(order_list) == 0:
                logger.warning(f"⚠️ {stock_code} 주문내역 조회 실패 또는 주문 없음")
                return False
            
            # 🔥 2. 가장 최근 체결된 매수 주문 찾기
            ...
            for order in order_list:
                # 오늘 매수 주문 중 체결 완료된 것
                if (order['OrderSide'] == 'Buy' and 
                    order['OrderSatus'] == 'Close' and
                    order['OrderDate'] == today_str):
                    
                    # 수량이 예상과 비슷한지 확인 (±10% 허용)
                    executed_amount = order['OrderResultAmt']
                    if abs(executed_amount - expected_amount) <= max(1, expected_amount * 0.1):
                        latest_buy_order = order
                        break
            
            if not latest_buy_order:
                logger.warning(f"⚠️ {stock_code} 해당하는 체결 주문을 찾을 수 없음 (예상수량: {expected_amount})")
                return False
            
            # 🔥 3. 실제 체결가 추출
            actual_execution_price = float(latest_buy_order['OrderAvgPrice'])
            executed_amount = latest_buy_order['OrderResultAmt']
            order_time = latest_buy_order['OrderTime']
            ...
            target_position['CurrentAmt'] = executed_amount  # 실제 체결량도 정확히
            target_position['EntryAmt'] = executed_amount    # EntryAmt도 동기화
                ...
                for stock in my_stocks:
                    if stock['StockCode'] == stock_code:
                        ...
                        if actual_executed >= amount:  # 목표 수량 이상 체결
                            ...
                            logger.info(f"   💰 체결가격: ${current_avg_price:.2f}")
                            
                            # 가격 개선 계산
                            execution_diff = current_avg_price - order_price
                            total_investment = current_avg_price * actual_executed  # 🔥 수정: 실제 체결량 기준
                            ...
                            if config.config.get("use_discord_alert", True):
                                ...
                                msg += f"💰 ${current_avg_price:.2f} × {actual_executed}주\n"  # 🔥 실제 체결량
                            ...
                            try:
                                # 현재 몇 차수 매수인지 파악
                                current_position_num = self.get_current_buying_position(stock_code)
                                if current_position_num:
                                    logger.info(f"🔄 {stock_name} {current_position_num}차 실제 체결가 동기화 시작")
                                    # self.sync_position_after_buy(
                                    #     stock_code=stock_code,
                                    #     position_num=current_position_num, 
                                    #     order_price=order_price,  # 주문가격 전달
                                    #     expected_amount=actual_executed  # 실제 체결량 전달
                                    # )
                                    # 🔥🔥🔥 새로운 동기화 로직 사용 🔥🔥🔥
    ...
    def should_buy_with_comprehensive_score(self, stock_code, position_num, indicators, 
        ...
        try:
            ...
            # 🔥 4단계: 상세 로깅 (설정 파일 우선 적용 정보 명확화)
            status = "✅ 매수" if final_decision else "❌ 대기"
            
            # 설정 적용 상태 표시
            config_source = "설정파일 우선"
            if ai_bonus > 0:
                config_source += f" + AI보너스(-{ai_bonus}점)"
            if safety_penalty > 0:
    ...
    def update_position_after_buy(self, stock_code, position_num, executed_amount, actual_price, magic_data_list):
        ...
        try:
            ...
            if is_reentry:
                ...
                logger.info(f"✅ {stock_code} {target_position_num}차 재진입 데이터 초기화 완료")
    ...
    def process_trading(self):
        """AI 봇 3차수 매매 로직 처리 - NVDA + PLTR 최적화 버전"""
        ...
        #     if time_diff > 1800:  # 30분마다
        #         logger.info("🔄 AI 봇 정기 전체 포지션 동기화 실행")
        #         self.sync_all_positions_with_broker()
        #         self.last_full_sync_time = current_time
        ...
        try:
            if NEWS_ANALYSIS_AVAILABLE:
                ...
                if news_summary is None:
                    ...
                    logger.info("📰 AI 봇 뉴스 API 호출 - 새로운 분석 수행")
                    news_summary = self.analyze_all_stocks_news()
                    self.cache_news_summary(news_summary)
                    
                    # API 호출 알림 (비용 모니터링용)
                    api_call_msg = f"💰 AI 봇 뉴스 API 호출됨 - {datetime.now().strftime('%H:%M:%S')}"
                    logger.warning(api_call_msg)
                    
                else:
                    # 캐시된 결과 사용 (API 비용 절약)
                    logger.info("📰 AI 봇 캐시된 뉴스 분석 결과 사용 (API 비용 절약)")
            else:
                news_summary = {}
        ...
        # 각 종목별 처리 - config 사용 (AI 봇에서는 기존 config 객체 사용)
        target_stocks = config.target_stocks
        ...
        for stock_code, stock_info in target_stocks.items():
            try:
                ...
                if stock_data_info is None:
                    ...
                    msg = f"🤖 {stock_code} AI 봇 3차수 투자 준비 완료!!"
                    logger.info(msg)
                    if config.config.get("use_discord_alert", True):
                        discord_alert.SendMessage(msg)
                ...
                for i, magic_data in enumerate(magic_data_list):
                    if not magic_data['IsBuy'] and i < 3:  # 0, 1, 2 (1차, 2차, 3차)
                        ...
                        # 🔥🔥🔥 매수 실행 로직 🔥🔥🔥
                        if should_buy:
                            logger.info(f"💰 {stock_code} {position_num}차 매수 진행 - AI 봇 3차수 시스템")
                            ...
                            if safety_check:
                                ...
                                if total_cost <= available_cash:
                                    ...
                                    if actual_price and executed_amount:
                                        # 🔥🔥🔥 AI 봇 3차수 데이터 업데이트 🔥🔥🔥
                                        logger.info(f"🔄 {stock_code} {position_num}차 AI 봇 매수 데이터 업데이트 시작")
                                        ...
                                        try:
                                            ...
                                            if update_success:
                                                logger.info(f"  📊 AI 봇 업데이트: {executed_amount}주 @ ${actual_price:.2f}")
                                                
                                                # 저장 시도
                                                self.save_split_data()
                                                logger.info(f"  💾 {stock_code} {position_num}차 AI 봇 데이터 저장 완료")
                                                ...
                                                # 성공 메시지
                                                msg = f"🤖 {stock_code} AI 봇 {buy_reason}!\n"
                                                ...
                                                logger.error(f"  ❌ {stock_code} {position_num}차 AI 봇 데이터 업데이트 실패")
                                                continue
                                            
                                        except Exception as update_e:
                                            logger.error(f"  ❌ {stock_code} {position_num}차 AI 봇 데이터 업데이트 중 오류: {str(update_e)}")
                                            ...
                                            if 'backup_data' in locals():
                                                try:
                                                    ...
                                                    logger.warning(f"  🔄 {stock_code} {position_num}차 AI 봇 롤백 완료")
                                                except:
                                                    logger.error(f"  💥 {stock_code} {position_num}차 AI 봇 롤백도 실패")
                                            continue
                                    
                                    else:
                                        # 매수 실패 (체결 실패)
                                        logger.warning(f"❌ {stock_code} {position_num}차 AI 봇 매수 실패: {message}")
                                        if "가격 급등" in message:
                                            logger.info(f"  💡 {stock_code} 가격 급등으로 인한 매수 포기는 정상적인 보호 기능입니다")
                                    
                                else:
                                    logger.warning(f"❌ {stock_code} AI 봇 매수 자금 부족: 필요 ${total_cost:.2f} vs 가용 ${available_cash:.2f}")
                            else:
                ...
                if holdings['amount'] > 0:
                    ...
                    if abs(internal_total - holdings['amount']) > 0:
                        logger.warning(f"{stock_code} AI 봇 수량 불일치 감지: 내부관리={internal_total}, API조회={holdings['amount']}")
                        if internal_total > 0:
                            sync_ratio = holdings['amount'] / internal_total
                            for magic_data in magic_data_list:
                                if magic_data['IsBuy']:
                                    magic_data['CurrentAmt'] = int(magic_data['CurrentAmt'] * sync_ratio)
                            logger.info(f"{stock_code} AI 봇 수량 동기화 완료: 비율={sync_ratio:.3f}")
                            self.save_split_data()
                    ...
                    # 매도 실행 여부만 로깅
                    if sells_executed:
                        logger.info(f"🎯 {stock_code} AI 봇 3차수 매도 전략 실행 완료")
                    else:
                        ...
                        if total_positions > 0:
                            logger.debug(f"💎 {stock_code} AI 봇 전체 {total_positions}주 홀딩 유지")

                # 🔥 간단한 API 호출 간격 추가
                time.sleep(0.5)  # 0.5초 대기
                
            except Exception as e:
                logger.error(f"{stock_code} AI 봇 처리 중 오류 발생: {str(e)}")
                import traceback
                traceback.print_exc()
    ...
    def check_hybrid_protection(self, stock_code, magic_data, current_price, position_return_pct, position_max):
        """하이브리드 보호 시스템 체크 - 🔥 설정값 기반 트레일링 조건 개선"""
        try:
            stock_config = config.target_stocks.get(stock_code, {})
            partial_config = stock_config.get('partial_sell_config', {})
            hybrid_config = partial_config.get('hybrid_protection', {})
            ...
            # 🔥🔥🔥 핵심 개선: 설정값 기반 트레일링 조건 🔥🔥🔥
            min_profit_for_trailing = hybrid_config.get('min_profit_for_trailing', 3)  # 🔥 설정값 읽기
            ...
            # 2단계: 부분매도 후 트레일링 (기존 유지 - 수익 상황에서만)
            elif current_stage >= 1:
                post_trailing_drop = hybrid_config.get('post_partial_trailing', 0.05)
                
                # 🔥 핵심 개선: 설정값 기반 수익 상황에서만 트레일링 작동
                if (position_return_pct > min_profit_for_trailing and  # 🔥 설정값 사용!
                    position_max > min_profit_for_trailing + 2 and    # 🔥 설정값 + 2% 이상 경험 필요
                    position_return_pct <= position_max - (post_trailing_drop * 100)):
                    
                    return {
                        'action': 'post_partial_trailing',
                        'sell_ratio': 1.0,
                        'reason': f'부분매도후 트레일링 (최고{position_max:.1f}%→{post_trailing_drop*100:.0f}%하락)',
                        'type': 'post_partial_trailing'
                    }
            
            # 3단계: 응급 트레일링 (부분매도 실패시 백업) - 🔥 설정값 기반 강화
                ...
                emergency_drop = hybrid_config.get('emergency_trailing_drop', 0.08)
                
                # 🔥🔥🔥 핵심 개선: 설정값 기반 응급 트레일링 조건 3중 체크 🔥🔥🔥
                condition_1 = emergency_enable  # 응급 트레일링 활성화
                condition_2 = position_return_pct > min_profit_for_trailing  # 🔥 설정값 사용!
                condition_3 = position_max >= min_profit_threshold  # 최고점 12% 이상 경험
                condition_4 = position_return_pct <= position_max - (emergency_drop * 100)  # 하락폭 체크
                
                # 🔥 상세 로그로 디버깅 정보 제공
                logger.info(f"🔍 {stock_code} 응급 트레일링 조건 체크:")
                logger.info(f"   조건1 (활성화): {condition_1}")
                logger.info(f"   조건2 (현재수익 > {min_profit_for_trailing}%): {condition_2} (현재: {position_return_pct:.1f}%)")
                logger.info(f"   조건3 (최고점 >= {min_profit_threshold}%): {condition_3} (최고: {position_max:.1f}%)")
                logger.info(f"   조건4 (하락폭 체크): {condition_4}")
                
                if condition_1 and condition_2 and condition_3 and condition_4:
                    return {
                        'action': 'emergency_trailing',
                        'sell_ratio': 1.0,
                        'reason': f'응급트레일링 (현재{position_return_pct:.1f}%, 최고{position_max:.1f}%→{emergency_drop*100:.0f}%하락)',
                        'type': 'emergency_trailing'
                    }
                else:
                    # 🔥 조건 미충족 시 상세한 이유 제공
                    if not condition_2:
                        reason = f"현재 수익률 부족 ({position_return_pct:.1f}% < {min_profit_for_trailing}%)"
                    elif not condition_3:
                        reason = f"최고점 미달 ({position_max:.1f}% < {min_profit_threshold}%)"
                    elif not condition_4:
                        reason = f"하락폭 부족 (현재: {position_return_pct:.1f}%, 임계값: {position_max - (emergency_drop * 100):.1f}%)"
                    else:
                        reason = "응급 트레일링 비활성화"
                    
                    logger.info(f"✅ {stock_code} 응급 트레일링 차단: {reason}")
                    return {'action': 'hold', 'reason': reason}
            
            return {'action': 'hold', 'reason': '하이브리드 조건 미충족'}
            
        except Exception as e:
            logger.error(f"하이브리드 보호 체크 중 오류: {str(e)}")
    ...
    def process_hybrid_sell_record(self, stock_code, magic_data, sell_amount, current_price, position_return_pct, hybrid_action):
        """하이브리드 매도 기록 처리 - 기존 부분매도 시스템과 호환"""
...
def setup_news_analysis_schedule():
    ...
    try:
        ...
        news_setup_msg += "🔍 **분석 대상**: NVDIA, Palantir \n"
...
def run_bot():
    ...
    try:
        ...
        # 🔥 시작 시 예산 정보 출력
        logger.info(f"🚀 AI 특화 하이브리드 보호 시스템 봇 시작! (표준화 완료)")
        ...
        # 🔥 표준화: 타겟 종목 현황 출력
        logger.info(f"🎯 AI주식 타겟 종목 현황 (원전봇과 표준화):")
        for stock_code, stock_config in target_stocks.items():
        ...
        logger.error(f"AI 봇 실행 중 오류 발생: {str(e)}")

def send_startup_message():
    """AI 봇 시작 메시지 전송 - 🔥 AI 특화 하이브리드 보호 시스템 버전 (표준화)"""
    try:
        target_stocks = config.target_stocks
        
        msg = "🔥 AI 특화 하이브리드 보호 시스템 봇 시작! (표준화 완료)\n"
        msg += "=" * 60 + "\n"
        msg += f"💱 통화: USD (달러)\n"
        msg += f"💵 설정 예산: ${config.absolute_budget:,.0f} (소액 최적화)\n"
        msg += f"📊 예산 전략: AI 집중 투자 + 성과 기반 동적 조정\n"
        msg += f"🎯 차수 시스템: {config.div_num:.0f}차수 초고속 진입\n"
        
        # 🔥 AI 특화 하이브리드 보호 시스템 강조
        msg += f"\n🔥 **AI 특화 하이브리드 보호 시스템** (원전봇과 표준화)\n"
        msg += f"✅ AI 모멘텀 + 하이브리드 보호 완벽 결합\n"
        msg += f"✅ 초고속 포착 (NVDA 0.5%, PLTR 1% 조정)\n"
        msg += f"✅ 초고속 수익 확보 (2.5%부터 부분매도)\n"
        msg += f"✅ 소액 투자 특화 (1주부터 부분매도)\n"
        msg += f"✅ AI 변동성 완벽 대응 + 무제한 상승 참여\n"
        msg += f"✅ 응급 백업 시스템 + 이중 안전망\n"
        msg += f"✅ 원전봇과 표준화: weight 필드 통일 사용\n"
        
        msg += f"\n🎯 **AI 타겟 종목** ({len(target_stocks)}개) - 하이브리드 완전 적용:\n"
        
        ai_hybrid_descriptions = {
            "NVDA": ("모멘텀 즉시 포착", "2.5% 초고속매도(25%)", "3% 응급보호", "0.5% 조정→즉시 매수"),
            "PLTR": ("변동성 적극 활용", "4% 빠른매도(20%)", "4% 응급보호", "1% 조정→즉시 매수")
        }
        
        for stock_code, stock_config in target_stocks.items():
            # 🔥 표준화: weight 필드 안전 접근
            weight = stock_config.get('weight', 0)  # 원전봇과 동일
            allocated = config.absolute_budget * weight
            stock_type = stock_config.get('stock_type', 'ai')
            
            # 하이브리드 설정 정보 추출
            partial_config = stock_config.get('partial_sell_config', {})
            hybrid_config = partial_config.get('hybrid_protection', {})
            
            first_threshold = partial_config.get('first_sell_threshold', 0)
            first_ratio = partial_config.get('first_sell_ratio', 0) * 100
            emergency_drop = hybrid_config.get('emergency_trailing_drop', 0) * 100
            min_quantity = hybrid_config.get('min_quantity_for_partial', 1)
            
            description, partial_info, trailing_info, scenario = ai_hybrid_descriptions.get(
                stock_code, ("AI 하이브리드", "부분매도", "응급보호", "AI 특화")
            )
            
            msg += f"🎯 **{stock_config.get('name', stock_code)}** ({stock_code})\n"
            msg += f"   💰 {weight*100:.0f}% (${allocated:,.0f}) - {stock_type}\n"  # weight 사용
            msg += f"   🔥 {description}\n"
            msg += f"   💎 {partial_info} ({first_threshold}% 시작)\n"
            msg += f"   🛡️ {trailing_info} ({emergency_drop:.0f}% 트레일링)\n"
            msg += f"   📊 {scenario} (최소 {min_quantity}주)\n"
        
        # 🔥 표준화 완료 메시지
        msg += f"\n✅ **표준화 완료**:\n"
        msg += f"🔧 원전봇과 동일한 weight 필드 사용\n"
        msg += f"📊 설정 구조 통일로 유지보수성 향상\n"
        msg += f"🚀 KeyError 'weight' 문제 완전 해결\n"
        
        # 기존 메시지 계속...
        msg += f"\n💡 **AI 하이브리드 동작 방식**:\n"
        msg += f"🚀 모멘텀: 소폭 조정 즉시 포착 → 빠른 부분매도 → 무제한 상승\n"
        msg += f"💎 변동성: 작은 조정 활용 → 단계별 확보 → 트레일링 보호\n"
        msg += f"🚨 응급: 부분매도 실패 → 응급 트레일링 → 전량 보호\n"
        msg += f"⚡ 소액: 1주부터 부분매도 → 유연한 수익 관리\n"
        msg += f"🔄 회전: 빠른 확보 → 재진입 기회 → 기회 확대\n"
        ...
        logger.error(f"AI 봇 시작 메시지 전송 중 오류: {str(e)}")
