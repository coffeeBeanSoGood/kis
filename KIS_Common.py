...
def GetToken(dist = "REAL"):
    ...
    try:
        ...
        logger.error("Exception by First")
...
#    logger.info(index_v, stock.get_index_ticker_name(index_v))
                ...
                if AutoLimitData['Id'] == AutoOrderId:
                    ...
                    for my_stock in MyStockList:
                        if my_stock['StockCode'] == AutoLimitData['StockCode']:
                            stock_amt = int(my_stock['StockAmt'])
                            logger.info(f"{my_stock['StockName']} {stock_amt}")
                            break
                    ...
                    try:
                        ...
                        logger.info(f"{len(OrderList)} <--- Order OK!!!!!!")
    ...
    for botOrderPath in BotOrderPathList:
        ...
        for AutoLimitData in AutoOrderList:
            ...
            if AutoLimitData['BotName'] == bot_name:
                logger.info(f"DELETE {bot_name} ORDER ->{AutoLimitData['Id']}")
